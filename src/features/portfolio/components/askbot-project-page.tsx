...
const detailSections = [
    ...
    notes: [
      {
        title: "预设/常用问题",
        text:
          "示例问题用于解决用户「不知道怎么问」的问题，很多业务用户在进入产品时容易产生空白感，通过展示典型问题，可以帮助用户快速理解系统能力，并模仿示例完成第一次提问，降低使用门槛，熟悉后可更换为个人常用问题，提高功能利用率。",
      },
      {
        title: "推荐问题",
        text:
  ...
  },
  {
    title: "问题理解",
    body:
      "痛点：用户担心 AI 是否真正理解了自己的问题，尤其在自然语言中，同一句话可能存在大量歧义，AI 的理解过程不可见会导致用户无法验证模型意图。\n\n解决：问题理解层的核心目标，是将用户模糊、口语化、存在歧义的自然语言问题，逐步转化为结构化、可确认、可执行的数据分析意图。这一层不仅影响 SQL 生成准确率，更直接影响用户对 AI 的信任感。",
    followup: {
      title: "确认与纠错",
      body:
    ...
    notes: [
      {
        title: "思考过程白盒化",
        text:
          "以流式状态展示 AI 当前的分析过程，通过将模型思考、Agent 决策、工具调用、文档召回、执行状态等关键过程适度透明化，帮助用户建立对 AI 的理解感、控制感，解决 AI 推理过程不可见带来的等待焦虑与黑盒感。",
      },
      {
        title: "增强关键参数外显",
        text:
  ...
  },
  {
    title: "结果展示",
    body:
      "痛点：虽然成功生成了 SQL，但结果展示仍然停留在“纯表格输出”，用户很难快速获取真正有价值的信息。系统完成了“数据查询”，但没有完成“信息传达”。\n\n解决：采用 Markdown + HTML 混合渲染方案，兼顾 AI 内容生成的稳定性与 BI 场景下的数据可视化、交互操作能力。在回答展示部分增加企业用户高频使用的数据分析能力，如查看 SQL、下载 CSV、复制结果等，满足结果验证与二次分析需求。",
    ...
    notes: [
      ...
      },
      {
        title: "Markdown",
        text:
          "轻量、结构清晰非常适合承载 AI 生成的文本内容，能够快速将复杂的数据分析结果结构化呈现，提升内容的可读性与信息获取效率。",
      },
    ],
  },
  {
